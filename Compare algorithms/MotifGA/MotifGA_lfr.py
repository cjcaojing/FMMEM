...
for netNO in range(1):
    network_name = 'network7' + str(netNO)
    nmi_flag = 1 #0:关，1：开
    faim_flag = 3
    M ="M1"
    runs = 2 #独立运行次数
    ...
    G = nx.read_edgelist("data/LFRnetwork/LFR_1000/"+ network_name + ".txt", create_using=nx.Graph())
    G = G.to_undirected()
    Gi = ig.Graph.Read_Edgelist("data/LFRnetwork/LFR_1000/"+ network_name + ".txt")
    ...
    if nmi_flag==1:
        # 获得真实社区划分
        fo = pd.read_table(r"data/LFRnetwork/LFR_1000/real/" + network_name + "_groundtruth.txt",header = None)
        real_membership =[]
        for i in fo[0]:
