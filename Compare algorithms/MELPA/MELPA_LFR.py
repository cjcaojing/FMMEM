...
net = '6'
for netNO in range(9):
    network_name = "network"+str(net)+str(netNO)
    Mlist = {1:"M1",2:"M2",3:"M3",4:"M4",5:"M5",6:"M6",7:"M7",8:"M8"} #模体选择列表
    M_flag = Mlist[1]
    nmi_flag = 1 #0:关，1：开
    aerfa_list = [i/10 for i in range(11)]
    # 获取网络数据中的边列表，并根据其使用igraph创建网络
    G1 = nx.read_edgelist("data/LFRnetwork/LFR_3000_20/"+ network_name + ".txt", create_using=nx.Graph())
    G1 = G1.to_undirected()
    Gi = ig.Graph.Read_Edgelist("data/LFRnetwork/LFR_3000_20/"+ network_name +".txt")
    ...
    if nmi_flag==1:
        #真实社区划分
        fo = pd.read_table(r"data/LFRnetwork/LFR_3000_20/real/" + network_name+"_groundtruth.txt",header = None)
        real=[]
        for i in fo[0]:
            real.append(i) 
