...
for netName in range(netStar,netEnd+1):
    ...
    for netNO in range(3):
        ...
        ######### FFA算法参数 #########
        FF_k = 10  #FF_k:农田分块数,通常取值范围为[2,8]
        ...
        while (run < Independent_Runs):
            ...
            # for gen in range(Gen):
                ...
                if gen>Gen and (((abs(abs(best_in_history_Q[gen-70]) - best_Q) <= 1.0e-7) or (best_in_history_Q[gen-50] == best_Q))) or (gen+1) % Gen ==0:
                    stop_flag=True
