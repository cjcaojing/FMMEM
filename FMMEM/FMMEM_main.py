...
example2_network = path + r'/example2.txt'

## 功能网络
macaque_network = path + r'/macaque.txt'

test7_network = path + r'/test7.txt'
...
while (run < Independent_Runs):
    ...
    # 用于记录各个个体的状态,用于判断该个体是否需要执行社区合并操作
    XS_info = {}
    for N in range(NP): 
    ...
    # for gen in range(Gen):
        ...
        if gen>300 and (((abs(abs(best_in_history_Q[gen-70]) - best_Q) <= 1.0e-7) or (best_in_history_Q[gen-50] == best_Q))) or (gen+1) % Gen ==0:
            stop_flag=True
            if gen+1<Gen:
        ...
        # TCM_Flag=True if gen>200 else False
        # 更新pop,fit
        pop = FNCM_pop
        fit = FNCM_fit
        
        if (gen+1) % 50 ==0 or stop_flag:
            # 计算Q，Qg,Qc,QW
            ...
            FM = 1 - func.fit_Q(Compactness,motif_info,bestx,motif_adj,n,len(set(bestx)),W,m,"MC")
            print("M_Qw={},FM={}".format(M_Qw,FM)) 
...
# F_QW1 = func.fit_Q(Compactness,motif_info,bestx,motif_adj,n,len(set(bestx)),W,m,"MC")
# print("F_QW1=",1-F_QW1)
# bestx_tmp = copy.deepcopy(bestx)
# bestx_tmp[4]=bestx_tmp[1]
# F_QW2 = func.fit_Q(Compactness,motif_info,bestx_tmp,motif_adj,n,len(set(bestx_tmp)),W,m,"MC")
# print("F_QW2=",1-F_QW2)
