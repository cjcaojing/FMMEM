...
# LFR_path = r"data/LFRnetwork/LFR_1000_15"
...
# LFR
netStar,netEnd = 8,8
for netName in range(netStar,netEnd+1):
    networkNO = 10*netName
    print("\n------------ network{}-network{} ---------------".format(networkNO,networkNO+2))
    lfr_Qws,lfr_Maxnmis,lfr_nmis = [],[],[]
    std_Qws,std_Maxnmis,std_nmis = [],[],[]
    for netNO in range(3):
        lfr_no = networkNO + netNO
        ...
        Q_flag = Qlist[6] #模块度函数 Qw
        OA_name = 'SOS'
        OA = OAlist[OA_name] #优化策略选用
        OA_info = eval(OA_name+'_info') #
        # 独立运行运行次数
        Independent_Runs = 3 #本次实验独立运行次数
        ...
        # 程序运行
        # =============================================================================
        print("===net:{0}==n:{1}==node_rate:{2}==e:{3}==edge_rate:{4}=={5}:{6}==ave_degree:{7}===".format(network_name,n,node_coverage_rate,len(edge_all),edge_coverage_rate,M_flag,r,average_degrees))
        del degrees
        #初始化数据
        Qws,Maxnmis,nmis = [],[],[]
        # RITA = {}
        ...
        while (run < Independent_Runs):
            ...
            betternmm_number_E, betternmm_number_FNCM, betterLSCM_number, TCM_Flag = 0, 0, 0, False
            # 用于记录各个个体的社区数目及状态
            XS_info = {}
            for N in range(NP): 
                XS_info[N] = [0,0,False]  #XS_info[0]:当前个体的适应度值，XS_info[1]:收敛迭代次数，XS_info[2]:是否收敛
            ...
            # for gen in range(Gen):
                ...
                (FNCM_pop, FNCM_fit, betternmm_number, LSCM_Num) = func.FNCM_funcs(MB,Compactness,motif_info,node_nei_info,E_pop,E_fit,n,NP,motif_adj,me_adj,Q_flag,W,m,XS_info,TCM_Flag)
                ...
                if gen>300 and (((abs(abs(best_in_history_Q[gen-50]) - best_Q) <= 1.0e-7) or (best_in_history_Q[gen-30] == best_Q))) or (gen+1) % Gen ==0:
                    stop_flag=True
                    if (gen+1)<Gen:
                ...
                TCM_Flag=True if gen>150 else False    
                ...
                if (gen+1)%20==0 or stop_flag:
                    # 计算Q，Qg,Qc,QW
                    print("###### "+OA_name+"_"+ M_flag +"_" + Q_flag + "_FMMEMcm ######")
                    print("gen={}".format(gen+1))
                    print("c_count={}".format(c_len))
                    # print("max_cum={}".format(max_cum))
                    if NMIflag==1:
                        print("MAXNMI={},NMI={}".format(max(best_in_history_NMI[10:]),nmi))
                    best_Q_result = (1 - best_Q) if Q_flag == "MC" else best_Q
                    print("best_"+ Q_flag + "={}, bestN={}".format(best_Q_result,best_index))
                    print("betternmm_E={}, betternmm_FNCM={}, betterLSCM={}".format(betternmm_number_E, betternmm_number_FNCM, betterLSCM_number))
                    betternmm_number_E,betternmm_number_NMM, betterMeger_number = 0, 0, 0 
                    M_Qw=ig.GraphBase.modularity(Gi,bestx,ij_participate_motif_number_list) 
                    F_QW = func.fit_Q(Compactness,motif_info,bestx,motif_adj,n,len(set(bestx)),W,m,Q_flag)
                    F_QW = (1 - F_QW) if Q_flag == "MC" else F_QW
                    print("M_Qw={},F_QW={}".format(M_Qw,F_QW)) 
                    endAll = time.process_time()
                    print("spendTime={}s".format(int(endAll-startAll)))
                    startAll=endAll
        
                if stop_flag: 
                    bestx, bestNMI, best_index = func.find_bestX(FNCM_pop, FNCM_fit, best_Q, real_mem, NMIflag)
                    Qws.append(fit[best_index])
                    if NMIflag==1:  
                        Maxnmis.append(max(best_in_history_NMI[10:]))
                        nmis.append(bestNMI)
                        # 多指标
            ...
            run+=1
        lfr_Qws.extend(Qws)
        std_Qws.append(np.std(Qws))
        lfr_Maxnmis.extend(Maxnmis)
        std_Maxnmis.append(np.std(Maxnmis))
        lfr_nmis.extend(nmis)
        std_nmis.append(np.std(nmis))
    
        # =============================================================================
        # 输出结果
        # =============================================================================
        print("\n## FMMEM #### {} ### {} ####".format(network_name,M_flag))        
        print('QW_mean={},std={}, max={}'.format(round(np.mean(Qws),6), np.std(Qws), max(Qws)))
        if NMIflag==1:
            print('MAXNMI_mean={},std={}, max={}'.format(round(np.mean(Maxnmis),6), np.std(Maxnmis), max(Maxnmis)))
            print('NMI_mean={},std={}, max={}'.format(round(np.mean(nmis),6), np.std(nmis), max(nmis)))
            # print("\n########### 多指标计算 ###########")
            # print("Qw={},FM={},NMI={},ARI={},F1_score={}".format(round(np.mean(RITA['Qws']),6),round(np.mean(RITA['FMs']),6),round(np.mean(RITA['nmis']),6),round(np.mean(RITA['ARIs']),6),round(np.mean(RITA['F1s']),6))) 
    
    # =============================================================================
    # 输出结果
    # =============================================================================
    print("\n## FMMEM #### network{}-{} ### {} ####".format(networkNO,networkNO+2,M_flag))        
    print('lfr_QW_mean={},std={}, max={}'.format(round(np.mean(lfr_Qws),6), np.mean(std_Qws), max(lfr_Qws)))
    print('lfr_MAXNMI_mean={},std={}, max={}'.format(round(np.mean(lfr_Maxnmis),6), np.mean(std_Maxnmis), max(lfr_Maxnmis)))
    print('lfr_NMI_mean={},std={}, max={}'.format(round(np.mean(lfr_nmis),6), np.mean(std_nmis), max(lfr_nmis)))
